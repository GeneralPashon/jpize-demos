...
public class Options{
    ...
    private int maxFramerate = Jpize.primaryMonitor().getRefreshRate();
    private boolean fullscreen = false;
    private boolean showFps = false;
    private float mouseSensitivity = 0.5F;
    private String audioDevice = Jpize.audio().getCurrent().getName();
    ...
    public Options(Session session, String gameDirPath){
        ...
        resOptions = new Resource(gameDirPath + "jpize.tests.minecraft.options.txt", true);
    ...
    private void init(){
        ...
        if(availableAudioDevices != null && availableAudioDevices.contains(audioDevice))
            Jpize.audio().getDevice(audioDevice).makeCurrent();
        else{
            audioDevice = Jpize.audio().getCurrent().getName();
            save();
        }
    ...
    public void save(){
        ...
        out.println("jpize.tests.minecraft.audio.device : " + audioDevice);
